...
import { Search, Plus, Wifi, WifiOff, MoreVertical, Edit, Trash2, Package, User, Settings, BarChart3, ShoppingBag, ExternalLink, Eye, MessageCircle, Star, Filter, Archive } from 'lucide-react';
import { signOut } from 'firebase/auth';
import { auth, db } from './firebase';
import { ref, get, remove, onValue, set } from 'firebase/database';
import AddProductModal from './AddProductModal';
import { useNavigate } from 'react-router-dom';
import { getProductImageUrl, formatProductForDisplay } from './productHelpers';
...
const ProductCatalogueView = ({ user = null, userProfile = null }) => {
  ...
  const [filterBy, setFilterBy] = useState('all'); // includes 'archived'
    ...
    const unsubscribe = onValue(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          const sortedProducts = [...productsArray].sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
  ...
  // Close menus on outside click & ESC; track online/offline
  useEffect(() => {
  ...
  // Archive (soft-delete): sets status = 'deleted'
  const handleArchiveProduct = async (productId, productName) => {
    ...
    const prev = products;
    setProducts((cur) =>
      cur.map((p) => (p.productId === productId ? { ...p, status: 'deleted' } : p))
    );
    setShowProductMenu(null);
  ...
  // Hard delete (kept as an extra option)
  ...
  const getBadgeStyle = (q, featured = false, status) => {
    if (status === 'deleted') return { padding: '4px 8px', borderRadius: 12, fontSize: 12, fontWeight: 600, background: 'rgba(107,114,128,.12)', color: '#6b7280' };
    if (featured) return { padding: '4px 8px', borderRadius: 12, fontSize: 12, fontWeight: 600, background: 'rgba(255, 193, 7, 0.1)', color: '#f59e0b' };
    if (q === 0) return { padding: '4px 8px', borderRadius: 12, fontSize: 12, fontWeight: 600, background: 'rgba(239,68,68,.1)', color: '#dc2626' };
    if (q <= 5) return { padding: '4px 8px', borderRadius: 12, fontSize: 12, fontWeight: 600, background: 'rgba(245,158,11,.1)', color: '#d97706' };
  ...
  const getBadgeText = (q, featured = false, status) => {
    ...
    return q === 0 ? 'Out of Stock' : q <= 5 ? `${q} left` : `${q} in stock`;
  };

  // Derived stats (hide archived by default)
  ...
  const inStockProducts = visibleProducts.filter(p => p.quantity > 0).length;
  const lowStockProducts = visibleProducts.filter(p => p.quantity > 0 && p.quantity <= 5).length;
  ...
  // Filter and sort products (without mutating original arrays)
  useEffect(() => {
    let base = [...products];
    if (filterBy !== 'archived') {
      base = base.filter((p) => p.status !== 'deleted');
    } else {
      base = base.filter((p) => p.status === 'deleted');
    }
    ...
    if (filterBy !== 'all' && filterBy !== 'archived') {
      base = base.filter(p => {
        switch (filterBy) {
          case 'in-stock': return p.quantity > 0;
          case 'out-of-stock': return p.quantity === 0;
          case 'low-stock': return p.quantity > 0 && p.quantity <= 5;
  ...
  const styles = {
    ...
    // IMPORTANT: allow the dropdown to overflow outside the card
    ...
    menuButton: { background:'none', border:'none', cursor:'pointer', padding:6, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 0 0 1px rgba(0,0,0,.06)', background:'rgba(255,255,255,.7)' },
    // Improved menu: more padding, min width, and scroll if tall
    productMenu: { position:'absolute', top:'calc(100% + 8px)', right:0, background:'#fff', borderRadius:12, boxShadow:'0 12px 28px rgba(0,0,0,.18)', border:'1px solid rgba(0,0,0,.06)', minWidth:200, maxHeight:300, overflowY:'auto', zIndex:3000, padding:6 },
              ...
              {filteredProducts.map(p => (
                <div key={p.productId} style={styles.productCard} className="product-card">
                  <div style={{ position: 'relative' }}>
                    <img src={getProductImageUrl(p)} alt={p.name} style={styles.productImage}/>
                  ...
                  <div style={styles.productInfo}>
                    <div style={styles.productHeader}>
                      <div style={{ flex:1 }}>
                        ...
                        <p style={styles.productPrice}>{formatPrice(p.price)}</p>
                        {p.shortDescription && (
