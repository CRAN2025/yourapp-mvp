...
const StorefrontView = ({ user, userProfile }) => {
  ...
  const handleWhatsAppStatus = () => {
    // Fall back to storeShare if statusUpdate helper isn't defined
    const maker =
    ...
    <>
      ...
      <div style={styles.container}>
        ...
        <header style={styles.header}>
          <div style={styles.headerContent}>
            ...
            <nav style={styles.nav}>
              ...
              <button onClick={() => navigate('/storefront')} style={{...styles.navButton, ...styles.navButtonActive}}>
        ...
        <div style={styles.content}>
          ...
          <div style={styles.card}>
            <h3 style={{ margin:'0 0 16px', fontSize:18, fontWeight:700 }}>🏪 Store Information</h3>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:16 }}>
              <div>
                <div style={{ fontSize:12, opacity:0.7, marginBottom:4 }}>Store Name</div>
                <div style={{ fontSize:16, fontWeight:600 }}>{sellerData?.storeName || 'Not set'}</div>
              </div>
              <div>
                <div style={{ fontSize:12, opacity:0.7, marginBottom:4 }}>Business Category</div>
                <div style={{ fontSize:16, fontWeight:600 }}>{sellerData?.businessCategory || sellerData?.businessType || 'Not set'}</div>
              </div>
              <div>
                  ...
                  {sellerData?.whatsappNumber ? (
                    <>
                      ...
                      {!isValidPhoneE164(sellerData.whatsappNumber) && (
                        <span style={{
                          fontSize:10, background:'rgba(245,158,11,0.1)', color:'#d97706',
              ...
              <div>
                ...
                <div style={{ fontSize:16, fontWeight:600 }}>{sellerData?.location || 'Not set'}</div>
              </div>
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ fontSize:12, opacity:0.7, marginBottom:4 }}>Store Description</div>
                <div style={{ fontSize:14, lineHeight:1.5 }}>
                  {sellerData?.storeDescription || sellerData?.description || 'No description added yet'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{
            ...
            <h3 style={{ margin:'0 0 16px', fontSize:20, fontWeight:700 }}>🚀 Quick Actions</h3>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(200px, 1fr))', gap:16 }}>
              <button
                onClick={() => window.open(storefrontUrl, '_blank')}
                style={{
                  background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.3)',
                  borderRadius: '12px', padding: '16px', color: 'white', cursor: 'pointer',
          ...
          <div style={styles.card}>
            <h3 style={{ margin:'0 0 16px', fontSize:18, fontWeight:700 }}>💡 Store Optimization Tips</h3>
            <div style={{ display:'grid', gap:12 }}>
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontWeight: '600', marginBottom: '4px' }}>
                    {analytics.featuredProducts > 0 ? `Great! You have ${analytics.featuredProducts} featured products`
        ...
        {shareModalOpen && (
          <div style={styles.modal} onClick={() => setShareModalOpen(false)}>
            <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
              <h3 style={{ margin:'0 0 16px', fontSize:20, fontWeight:700 }}>📤 Share Your Store</h3>

              <div style={{ display:'grid', gap:12 }}>
                <button onClick={handleWhatsAppShare} style={{...styles.button, ...styles.whatsappButton, justifyContent:'flex-start'}} className="btn-hover">
                  <MessageCircle size={16} /> Share via WhatsApp Chat
                </button>

                <button onClick={handleWhatsAppStatus} style={{...styles.button, background:'#128C7E', color:'white', justifyContent:'flex-start'}} className="btn-hover">
                  <Share2 size={16} /> Post to WhatsApp Status
                </button>

                <button onClick={handleCopyLink} style={{...styles.button, ...styles.secondaryButton, justifyContent:'flex-start'}} className="btn-hover">
                  <Copy size={16} /> Copy Store Link
                </button>
              </div>

              <button onClick={() => setShareModalOpen(false)} style={{...styles.button, ...styles.secondaryButton, width:'100%', marginTop:16, justifyContent:'center'}}>
                Close
              </button>
            </div>
          </div>
